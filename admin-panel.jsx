...
const Icons = {
  ...
  image: <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>,
...
const AVATAR_COLORS = ["#6366f1","#ec4899","#f59e0b","#10b981","#ef4444","#3b82f6","#8b5cf6","#14b8a6","#f97316","#06b6d4"];

const DEFAULT_DATA = {
  thumbnails: [
    { id: "t1", title: "TOP 10 PLAYS", channel: "GamerPro", views: "1.2M", icon: "🎮", gradient: 0, category: "showcase1", channelAvatar: "GP" },
    { id: "t2", title: "iPhone 16 Review", channel: "TechDaily", views: "2.1M", icon: "💻", gradient: 2, category: "showcase1", channelAvatar: "TD" },
    { id: "t3", title: "Hidden Waterfalls", channel: "NatureVlogs", views: "450K", icon: "🌿", gradient: 3, category: "showcase1", channelAvatar: "NV" },
    { id: "t4", title: "30 Day Transform", channel: "FitLife", views: "670K", icon: "💪", gradient: 4, category: "showcase2", channelAvatar: "FL" },
    { id: "t5", title: "NEW ALBUM REACTION", channel: "MusicBox", views: "3.4M", icon: "🎵", gradient: 5, category: "showcase2", channelAvatar: "MB" },
    { id: "t6", title: "Gordon Ramsay CRIES", channel: "CookMaster", views: "1.8M", icon: "🍳", gradient: 6, category: "showcase2", channelAvatar: "CM" },
  ],
  creators: [
  ...
  stats: [
    { id: "s1", value: "200M+", label: "Views Generated" },
    { id: "s2", value: "1500+", label: "Thumbnails Made" },
    { id: "s3", value: "200+", label: "Happy Creators" },
  ],
  process: [
  ...
  faq: [
    { id: "f1", q: "How fast do you deliver?", a: "Standard 24-48 hours. Express within 12 hours." },
    { id: "f2", q: "What are your prices?", a: "Thumbnails from $50. Monthly from $300. Branding from $800." },
    { id: "f3", q: "How many revisions?", a: "2-3 rounds included." },
  ],
  ...
  siteSettings: { brandName: "Studio", heroHeadline: "Crafting Instant Clicks.", heroSub: "My work isn't just good-looking — it's about making visuals that pull people in.", email: "hello@studio.com" },
};
...
export default function AdminPanel() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await window.storage.get("admin-data");
  ...
  const save = async (newData) => {
    setData(newData);
    try { await window.storage.set("admin-data", JSON.stringify(newData)); } catch(e) {}
    showToast("Uloženo ✓");
  };
  ...
  const tabs = [
    ...
    { id: "socials", label: "Socials", icon: Icons.link, count: data.socials.length },
    { id: "settings", label: "Nastavení", icon: Icons.settings },
  ];

  const deleteItem = (section, id) => {
    const newData = { ...data, [section]: data[section].filter(x => x.id !== id) };
    save(newData);
  };

  const addItem = (section, item) => {
    const newData = { ...data, [section]: [...data[section], { ...item, id: uid() }] };
    save(newData);
    setModal(null);
  };

  const updateItem = (section, id, updates) => {
    const newData = { ...data, [section]: data[section].map(x => x.id === id ? { ...x, ...updates } : x) };
    save(newData);
    setModal(null);
  };
    ...
    <div style={{ display: "flex", height: "100vh", background: "#0a0a0c", color: "#fff", fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", fontSize: "14px" }}>
      ...
      <aside style={{ width: 240, background: "#111114", borderRight: "1px solid rgba(255,255,255,.06)", display: "flex", flexDirection: "column", flexShrink: 0 }}>
        <div style={{ padding: "20px 20px 16px", borderBottom: "1px solid rgba(255,255,255,.06)" }}>
          <div style={{ fontWeight: 800, fontSize: 18 }}>Studio<span style={{ color: "#10b981" }}>.</span></div>
        ...
        <nav style={{ flex: 1, padding: "12px 10px", overflowY: "auto" }}>
          {tabs.map(t => (
            <button key={t.id} onClick={() => { setActiveTab(t.id); setSubFilter("all"); }}
              style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderRadius: 8, border: "none", background: activeTab === t.id ? "rgba(16,185,129,.1)" : "transparent", color: activeTab === t.id ? "#10b981" : "#8a8a9a", cursor: "pointer", fontFamily: "inherit", fontSize: 13, fontWeight: 600, marginBottom: 2, transition: "all .2s", textAlign: "left" }}>
              {t.icon}
        ...
        <div style={{ padding: 16, borderTop: "1px solid rgba(255,255,255,.06)", fontSize: 11, color: "#55555f" }}>
          v1.0 — Portfolio Admin
        </div>
      </aside>
      ...
      <main style={{ flex: 1, overflowY: "auto", padding: 32 }}>
        ...
        {activeTab === "creators" && <CrudSection title="Tvůrci" subtitle="Trusted by — scrollující strip avatarů" items={data.creators}
          onDelete={(id) => deleteItem("creators", id)}
          onAdd={() => setModal({ type: "creator" })}
          renderItem={(c) => (
        ...
        {activeTab === "process" && <CrudSection title="Process" subtitle="3 kroky designového procesu" items={data.process}
          onDelete={(id) => deleteItem("process", id)}
          onAdd={() => setModal({ type: "process" })}
          renderItem={(p, i) => (
            <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
              <div style={{ width: 36, height: 36, borderRadius: "50%", background: "#10b981", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, flexShrink: 0 }}>{i + 1}</div>
        ...
        {activeTab === "socials" && <CrudSection title="Socials" subtitle="Odkazy na sociální sítě" items={data.socials}
          onDelete={(id) => deleteItem("socials", id)}
          onAdd={() => setModal({ type: "social" })}
          renderItem={(s) => (
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ fontWeight: 700 }}>{s.platform}</div>
              <div style={{ fontSize: 12, color: "#10b981", wordBreak: "break-all" }}>{s.url}</div>
            </div>
          )}
      ...
      {/* TOAST */}
      {toast && <div style={{ position: "fixed", bottom: 24, right: 24, background: "#10b981", color: "#fff", padding: "10px 20px", borderRadius: 10, fontWeight: 700, fontSize: 13, zIndex: 9999, animation: "fadeIn .3s ease" }}>{toast}</div>}
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 24, marginTop: 16 }}>
        {[["all","Vše"],["showcase1","Showcase #1"],["showcase2","Showcase #2"]].map(([k,l]) => (
          <button key={k} onClick={() => setSubFilter(k)}
            style={{ padding: "8px 18px", borderRadius: 100, border: "1px solid " + (subFilter === k ? "#10b981" : "rgba(255,255,255,.06)"), background: subFilter === k ? "rgba(16,185,129,.1)" : "transparent", color: subFilter === k ? "#10b981" : "#8a8a9a", cursor: "pointer", fontFamily: "inherit", fontSize: 12, fontWeight: 600 }}>
        ...
        {filtered.map(t => (
          <div key={t.id} style={{ borderRadius: 12, overflow: "hidden", border: "1px solid rgba(255,255,255,.06)", background: "#161619", transition: "all .3s" }}>
            <div style={{ height: 140, background: GRADIENTS[t.gradient % 10], display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
              ...
              <div style={{ position: "absolute", top: 8, right: 8, background: t.category === "showcase1" ? "#10b981" : "#6366f1", padding: "3px 10px", borderRadius: 6, fontSize: 10, fontWeight: 700 }}>
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: "#161619", border: "1px solid rgba(255,255,255,.08)", borderRadius: 16, padding: 28, width: 440, maxHeight: "80vh", overflowY: "auto" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {type === "thumbnail" && <>
            ...
            <Field label="Ikona (emoji)" value={form.icon} onChange={v => set("icon", v)} placeholder="🎮" />
            <div>
              <div style={labelStyle}>Kategorie</div>
              <div style={{ display: "flex", gap: 8 }}>
                {[["showcase1","Showcase #1 (s profilkou)"],["showcase2","Showcase #2 (jen obrázky)"]].map(([k,l]) => (
                  <button key={k} onClick={() => set("category", k)} style={{ flex: 1, padding: "10px", borderRadius: 8, border: "1px solid " + (form.category === k ? "#10b981" : "rgba(255,255,255,.06)"), background: form.category === k ? "rgba(16,185,129,.1)" : "transparent", color: form.category === k ? "#10b981" : "#8a8a9a", cursor: "pointer", fontFamily: "inherit", fontSize: 12, fontWeight: 600 }}>{l}</button>
                ))}
              </div>
            </div>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        {GRADIENTS.map((g, i) => (
          <button key={i} onClick={() => onChange(i)}
            style={{ width: 36, height: 36, borderRadius: 8, background: g, border: value === i ? "2px solid #10b981" : "2px solid transparent", cursor: "pointer", transition: "all .2s" }} />
        ))}
      </div>
    </div>
  );
...
function getDefaults(type) {
  const d = {
    thumbnail: { title: "", channel: "", channelAvatar: "", views: "", icon: "🎮", gradient: 0, category: "showcase1" },
...
const addBtnStyle = { display: "inline-flex", alignItems: "center", gap: 8, padding: "10px 20px", borderRadius: 10, border: "none", background: "#10b981", color: "#fff", fontFamily: "inherit", fontSize: 13, fontWeight: 700, cursor: "pointer" };
